...
parser.add_argument("-f", "--frame_length", type=int, nargs=1,
                    metavar="boundary", default=15000,
...
def muscle_align_sequences(fasta_filename, enzyme):
    ...
    muscle_cmd = ["muscle", "-in", fasta_filename, "-out", f"{fasta_filename}_aligned.fasta", "-seqtype", "protein", "-maxiters", "16", "-gapopen", str(enzymes[enzyme]["gap_opening_penalty"]), "-gapextend", str(enzymes[enzyme]["gap_extend_penalty"])]
            ...
            with pyhmmer.plan7.HMMFile(enzyme_hmm_filename) as hmm_file:
                ...
                try:
                    ...
                    for hit in pipeline.search_hmm(hmm, sequences):
                            ...
                            if feature:
                                results[get_identifier(feature)] = extract_feature_properties(feature)
                except Exception as e:
        ...
        for feature in record.features:
            if feature.type == "CDS":
                try:
                    ...
                    print("Error processing feature:", e, "in record", record.id, "for feature", feature)
                    continue
...
def process_dataframe_and_save(complete_dataframe, gb_record, trailing_window, output_path, score_threshold=0, target_BGC_type="RiPPs"):
    ...
    for index, row in complete_dataframe.iterrows():
        ...
        score = 0
        protein_details = []  # List to store information about proteins and their attributes
