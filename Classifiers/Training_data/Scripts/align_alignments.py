...
def adjust_alignments_using_msa(input_dir, reference_id="Reference"):
    ...
    references = [next(SeqRecord(rec.seq, id = f"{rec.id}_{index}") for rec in align if rec.id == reference_id) for index, align in enumerate(alignments)]

    temp_file = f"temp_references{input_dir}.fasta"
    with open(temp_file, "w") as f:
        for index, ref in enumerate(references):
            f.write(f">{ref.id}\n{str(ref.seq)}\n")

    # Align using MUSCLE
    output_file = f"muscle_aligned_references{imput_dir}.fasta"
    ...
    index_to_record = {int(record.id.split('_')[-1]): record for record in aligned_references}
    ...
    for idx, alignment in enumerate(alignments):
        adjusted_alignment = _adjust_alignment_based_on_reference(alignment, references[idx], reordered_alignment[idx].seq)
        final_adjusted_alignment = []
        for record in adjusted_alignment:
            final_seq = str(record.seq).replace('X', '-')
            final_record = SeqRecord(Seq(final_seq), id=record.id, description=record.description)
        ...
        with open(adjusted_filename, "w") as output_file:
            AlignIO.write(MultipleSeqAlignment(final_adjusted_alignment), output_file, "fasta")
