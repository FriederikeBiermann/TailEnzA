...
def split_alignment(alignment,fragment,fastas_aligned_before):
    ...
    if fragment[0]!="end":
        for record in alignment:
            subsequence=str(record.seq)[start-1:end].replace('-', '') #die leerzeichen wurden hier wieder rausgelöscht aus den alignments und als subsequence gespeichert
...
def fragment_alignment(alignment,splitting_list, fastas_aligned_before):
    ...
    fragment_matrix=pd.DataFrame() #erstellt leeren pandas Dataframe
    if fastas_aligned_before==False:

        seqa=alignment[0] #=reference_sequence
        seqb=alignment[1]
