...
class CNN(nn.Module):
    def __init__(self, total_features: int, num_fragments: int, num_classes: int):
        ...
        self.features_per_fragment = (total_features - num_fragments - 1) // num_fragments
        assert (total_features - num_fragments - 1) % num_fragments == 0, "Total features do not evenly divide into fragments"
        ...
        self.fc1_input_dim = 64 * (self.num_fragments // 4) * (self.features_per_fragment // 4)
    ...
    def forward(self, x):
        ...
        batch_size = x.size(0)
        main_features = main_features.view(batch_size, 1, self.num_fragments, self.features_per_fragment)
...
class RNN(nn.Module):
    def __init__(self, in_features: int, hidden_size: int, num_fragments: int, num_classes: int):
        super(RNN, self).__init__()
        self.hidden_size = hidden_size
        self.num_fragments = num_fragments
        self.features_per_fragment = (in_features - num_fragments - 1) // num_fragments + 1
        assert (in_features - num_fragments - 1) % num_fragments == 0, "Total features do not evenly divide into fragments"
