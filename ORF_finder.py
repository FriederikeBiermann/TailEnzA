        ...
        for frame in range(3):
            ...
            for aa_end in range(0, trans_len):
                if trans[aa_end] == "M" or trans[aa_end] == "V":
                    ...
                    if not any(f.type == "CDS" and orf_loc.start >= f.location.start and orf_loc.end <= f.location.end for f in record.features) and len(trans[aa_start:aa_end])>2:
...
with open(output_file, "w") as f:
    for orf in orfs:
        seq_id = f">{args.input_file}|{orf.location.start}|{orf.location.end}|{orf.location.strand}"
